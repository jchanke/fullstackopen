...
const loginAs = async (api, { username, password }) => {
  const { body: { token: token } } = await api
    .post('/api/login')
  ...
  describe('addition of a new blog', () => {
    ...
    describe('when user is logged in', async () => {
      ...
      test('succeeds with valid data', async () => {
        ...
        const token = await loginAs(api, { username: 'hwang', password: 'hwang' })
      ...
      test('`likes` field defaults to 0 if not given', async () => {
        ...
        const token = await loginAs(api, { username: 'srabin', password: 'srabin' })
  ...
  describe('deletion of a blog', () => {
    ...
    describe('when user is logged in', () => {
      ...
      test('succeeds with statuscode 204 if id is valid', async () => {
        ...
        const token = await loginAs(api, { username: user.username, password: user.username })
      ...
      test('fails with statuscode 401 if wrong user is logged in', async () => {
        ...
        const userId = blogToDelete.user.toString()
        const otherUsers = (await helper.usersInDb())
